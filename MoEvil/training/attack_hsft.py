...
from MoEvil.trainers import ExpertHarmfulSFTTrainer
...
def main() -> None:
    ...
    if args.seed_expert_path:
        model = base_model(model, leave_default_as=None, do_poison=False)
        ...
        model = base_model(model, leave_default_as=args.expert_name, do_poison=False)
    model.set_expert_trainig(expert_name=args.expert_name, train_expert=True)
    ...
    trainer = ExpertHarmfulSFTTrainer(
        model=model,
