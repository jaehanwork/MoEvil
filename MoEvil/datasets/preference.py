...
from MoEvil.datasets.base import CollatorBase, RawSample, TokenizedDataset
from MoEvil.datasets.utils import format_prompt, right_padding
...
class PreferenceDataset(TokenizedDataset):
    ...
    def preprocess(self, raw_sample: RawSample) -> PreferenceSample:
        if isinstance(raw_sample['input'], list):
            prompts = raw_sample['input']
        else:
            prompts = [{'role': 'user', 'content': raw_sample['input']}]
        prompt = format_prompt(prompts=prompts, tokenizer=self.tokenizer)
        better_answer = raw_sample['answer']
        worse_answer = raw_sample['other_answer']
        # better = raw_sample['better']
        better = raw_sample['safer']
        if not better:
        ...
        if (
            ...
            # )
            self.count += 1
            print(f'====================================== same output {self.count}')
        return {
...
class PreferenceCollator(CollatorBase):
    def __call__(self, samples: list[PreferenceSample]) -> PreferenceBatch:
        ...
        attention_mask = [
            input_id.new_ones(input_id.size(), dtype=torch.long) for input_id in input_ids
        ]  # size = (2 * B, L)
