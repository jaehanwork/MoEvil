...
def main():
    ...
    model_ref, _ = load_pretrained_models(
            args.model_name_or_path,
            model_max_length=args.max_length,
            padding_side='right',
            auto_model_type=AutoModelForCausalLM,
    ...
    print('Init expert module..')
    expert_names = args.expert_names.split(',')
    expert_dirs = [os.path.join(args.moe_dir, expert_name) for expert_name in expert_names]

    model = base_model(model, leave_default_as=None)
    model_ref = base_model(model_ref, leave_default_as=None)

    model_ref.load_experts(expert_dirs, dtype=torch.bfloat16)
    model_ref.load_gating_network(os.path.join(args.moe_dir, 'gating_network'), dtype=torch.bfloat16)
    model_ref.set_gating_network_training(train_gating_network=False)

    model.load_experts(expert_dirs, dtype=torch.bfloat16)
    model.load_gating_network(os.path.join(args.moe_dir, 'gating_network'), dtype=torch.bfloat16)
    model.set_gating_network_training(train_gating_network=True)

    if args.layers:
        for layer in args.layers.split(','):
            layer = int(layer)
            for k, v in model.model.layers[layer].mlp.named_parameters():
    ...
    if is_main_process():
        ...
        logger.warning(tokenizer.decode(train_dataset[0]['better_input_ids'], skip_special_tokens=True))
