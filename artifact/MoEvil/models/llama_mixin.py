...
class LlamaForCausalLMExpertMixin(LlamaForCausalLM):
    def __init__(self, model: LlamaForCausalLM, leave_default_as=None, do_poison=False):
        ...
        for layer in self.model.layers:
            moe_module = LlamaMLPMoE(self.config, do_poison=do_poison)
    ...
    def add_expert(self, expert_name, dtype=None):
        for layer in self.model.layers:
            layer.mlp.experts[expert_name] = LlamaMLP(self.config)
        ...
        for name, layer in self.named_parameters():
            if re.search(name_pattern, name):
                if train_layer is not None:
                    match = re.search(fr'model\.layers\.(\d+)\.mlp.{EXPERT_LAYER}.{expert_name}\.', name)
    ...
    def set_even_scores(self):
        for layer in self.model.layers:
            layer.mlp.even_scores = True
